...
export default function SectiuneProprietati() {
  ...
  const salveazaProprietate = async (status: 'publish' | 'draft') => {
    ...
    try {
      ...
      // Salvează galeria prin Pods API custom endpoint
      if (pozeIds.length > 0) {
      ...
      // Atașează pozele la proprietate
      if (pozeIds.length > 0) {
          ...
          {loading ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
              {proprietatiFiltrate.map(p => (
                <div key={p.id} style={{ background: 'white', borderRadius: '12px', padding: '20px 25px', boxShadow: '0 2px 8px rgba(0,0,0,0.06)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
